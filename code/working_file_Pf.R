...
pf_abundance_go <- read_excel(path="raw_data/ProteinExpressionAndGo_RawAbundance.xlsx", 
                               sheet=2,
                               col_types=c(GI_Number = "numeric", RefSeq_Accession = "text", Accession = "text",
                                           Description = "text", Description_noGI = "text", Contaminant = "logical",
...
# Remove RefSeq_Accession and Description_noGI columns for now since the Pst file doesn't have those columns
pf_abundance_go <- select(pf_abundance_go, -"refseq_accession")
pf_abundance_go <- select(pf_abundance_go, -"description_nogi")


#how many missing values per row of each condition
pf_complete_na <- rowSums(is.na(dplyr::select(pf_abundance_go, gi_number, description, pf_complete_1, pf_complete_2, pf_complete_3)))
pf_minimal_na <- rowSums(is.na(dplyr::select(pf_abundance_go, gi_number, description, pf_minimal_1, pf_minimal_2, pf_minimal_3)))

#append these lists to main table
#if 2 or more missing values, add to new data table "differential"
pf_abundance_go <- mutate(pf_abundance_go, pf_comp_na = pf_complete_na, pf_min_na = pf_minimal_na)
differential <- data.frame(filter(pf_abundance_go, pf_comp_na >= 2 | pf_min_na >= 2))
...
# test for normality
for(i in 15:23){
  print(shapiro.test(pull(pf_abundance_go, i)))
}
...
test <- numeric()
for(i in 15:23){
  newname <- c(newname, paste("log_", colnames(pf_abundance_go)[[i]], sep = "_"))
  pf_abundance_go <- mutate(pf_abundance_go,
                             "{newname[[i-14]]}" := log2(pf_abundance_go[[i]]))
  test <- c(test, print(shapiro.test(pull(pf_abundance_go, newname[[i-14]]))))
}
...
pf_abundance_go <- mutate(pf_abundance_go, avg_log_pf_complete = 
                             pf_abundance_go %>% 
                             dplyr::select(31:33) %>% 
                             rowMeans())

pf_abundance_go <- mutate(pf_abundance_go, mc_log_pf_complete_1 = pf_abundance_go$log__pf_complete_1 / summary(pf_abundance_go$avg_log_pf_complete)[["Mean"]])
pf_abundance_go <- mutate(pf_abundance_go, mc_log_pf_complete_2 = pf_abundance_go$log__pf_complete_2 / summary(pf_abundance_go$avg_log_pf_complete)[["Mean"]])
pf_abundance_go <- mutate(pf_abundance_go, mc_log_pf_complete_3 = pf_abundance_go$log__pf_complete_3 / summary(pf_abundance_go$avg_log_pf_complete)[["Mean"]])

pf_abundance_go <- mutate(pf_abundance_go, mc_log_pf_minimal_1 = pf_abundance_go$log__pf_minimal_1 / summary(pf_abundance_go$avg_log_pf_complete)[["Mean"]])
pf_abundance_go <- mutate(pf_abundance_go, mc_log_pf_minimal_2 = pf_abundance_go$log__pf_minimal_2 / summary(pf_abundance_go$avg_log_pf_complete)[["Mean"]])
pf_abundance_go <- mutate(pf_abundance_go, mc_log_pf_minimal_3 = pf_abundance_go$log__pf_minimal_3 / summary(pf_abundance_go$avg_log_pf_complete)[["Mean"]])

mc_log_for_plot <- pf_abundance_go %>% 
  dplyr::select(41:46) %>% 
...
pf_abundance_go <- mutate(pf_abundance_go, imp_mc_log_pf_complete_1 = imp_for_tests$mc_log_pf_complete_1)
pf_abundance_go <- mutate(pf_abundance_go, imp_mc_log_pf_complete_2 = imp_for_tests$mc_log_pf_complete_2)
pf_abundance_go <- mutate(pf_abundance_go, imp_mc_log_pf_complete_3 = imp_for_tests$mc_log_pf_complete_3)

pf_abundance_go <- mutate(pf_abundance_go, imp_mc_log_pf_minimal_1 = imp_for_tests$mc_log_pf_minimal_1)
pf_abundance_go <- mutate(pf_abundance_go, imp_mc_log_pf_minimal_2 = imp_for_tests$mc_log_pf_minimal_2)
pf_abundance_go <- mutate(pf_abundance_go, imp_mc_log_pf_minimal_3 = imp_for_tests$mc_log_pf_minimal_3)
...
pf_abundance_go <- mutate(pf_abundance_go, p_value_comp_vs_min = p_values)
...
for(i in 1:nrow(pf_abundance_go)){
  complete = c(pf_abundance_go[i, 47:49])
  minimal = c(pf_abundance_go[i, 50:52])
...
pf_abundance_go <- mutate(pf_abundance_go, avg_imp_mc_log_complete = avg_imp_mc_log_complete)
pf_abundance_go <- mutate(pf_abundance_go, avg_imp_mc_log_minimal = avg_imp_mc_log_minimal)
...
pf_abundance_go <- mutate(pf_abundance_go, fc_min_vs_comp = vp$fc)
pf_abundance_go <- mutate(pf_abundance_go, nlog10_p_adj = vp$log_p_adj)
pf_abundance_go <- mutate(pf_abundance_go, log2_fc_min_vs_comp = vp$log2_fc)
...
hm_total <- pf_abundance_go %>% 
  dplyr::select(41:46)

rownames(hm_total) <- pf_abundance_go$description
...
hm_grouped <- select(pf_abundance_go, "avg_imp_mc_log_complete", "avg_imp_mc_log_minimal")
rownames(hm_grouped) <- pf_abundance_go$description
...
pf_pca <- pf_abundance_go %>%
  dplyr::select(31:39) %>% # 15:23 raw data, 31:39 log transformed
  drop_na() %>%
  prcomp(center = TRUE, scale. = TRUE) #scale. = TRUE


color <- case_when(grepl("complete", rownames(pf_pca$rotation)) ~ "complete", 
                   grepl("minimal", rownames(pf_pca$rotation)) ~ "minimal",
                   grepl("pool", rownames(pf_pca$rotation)) ~ "pool")

ggplot(as.data.frame(pf_pca$rotation), aes(x=PC2, y=PC3, color=color)) +
...
x_lab = paste("PC1", " (", (summary(pf_pca)$importance[2,1])*100, "%)", sep = "")
y_lab = paste("PC2", " (", (summary(pf_pca)$importance[2,2])*100, "%)", sep = "")
ggplot(as.data.frame(pf_pca$rotation), aes(x=PC1*100, y=PC2*100, color=color)) +
  geom_point(shape=19, size=2) +
  #coord_fixed() +
  labs(title=NULL,
       x=x_lab,
